...
{
    protected $_code = 'imaginationmedia_correios';
    ...
    {
        return ['imaginationmedia_correios' => "imaginationmedia_correios"];
    }
    ...
    public function collectRates(RateRequest $request)
    {
        ...
        foreach ($correiosMethods as $correiosMethod) {
            if ($correiosMethod["valor"] > 0 && $this->validateSameRegion($postcodeNumber, $correiosMethod['servico_codigo'])) {
                ...
                $method->setCarrier('imaginationmedia_correios');
                $method->setCarrierTitle($this->_scopeConfig->getValue(
                    'carriers/imaginationmedia_correios/name',
                    $this->_storeScope
                ));
                $method->setMethod('imaginationmedia_correios' . $correiosMethod['servico_codigo']);
